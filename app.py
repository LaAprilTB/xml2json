...
class MainWindow(QtWidgets.QMainWindow):
    ...
    YOLO_LABELS = ["HCN", "VCN", "PMN", "SEAL", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D9"]
    def __init__(
        self,
        config=None,
        filename=None,
    ...
    def xml2json(self):
        ...
        for filepath in glob.glob(os.path.join(path,'*.xml')):
            ...
            #img_file = os.path.join(path, os.path.splitext(xml_file)[0] + '.jpg')
            img_name = os.path.splitext(xml_file)[0] + '.jpg'
            json_file = os.path.join(path+'\json', os.path.splitext(xml_file)[0] + '.json')
            print(json_file)
            if os.path.isfile(filepath):
                ...
                for member in root.findall('object'):
                    ...
                    label_str = self.format_label(member[0].text)
                    bw = xmax - xmin
                    bh = ymax - ymin
                    flags = dict(PMN=False, VCN=False, HCN=False, PMPN=False)
                    if label_str not in self.YOLO_LABELS and bw > 0 and bh > 0:
                        if bw < bh:
                            flags["VCN"] = True
                        else:
                            flags['HCN'] = True
                    ...
                    if len(shapes) > 0:
                        print(json_file)
                        lf = LabelFile()
                        if os.path.dirname(json_file) and not os.path.exists(os.path.dirname(json_file)):
                            os.makedirs(osp.dirname(json_file))
                        lf.save(
                            filename=json_file,
                            shapes=shapes,
                            imagePath=img_name,
